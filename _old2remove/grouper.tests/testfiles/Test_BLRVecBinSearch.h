 ...
 *    include grouper/Test_BLRVecBinSearch.mk
 * Build the grouper/Test_BLRVecBinSearch.mk target from the Make Target view
 */

#ifndef TEST_BLRVecBinSearch
#define TEST_BLRVecBinSearch
...
class Test_BLRVecBinSearch : public CppUnit::TestFixture {
	BLRVecBinSearch blrBs;
	BLRExhaustSearch blrEs;

	CPPUNIT_TEST_SUITE(Test_BLRVecBinSearch);
...
public:
	...
	Test_BLRVecBinSearch(void) : blrBs(),blrEs() {}
...
protected:
	...
	void testsearchFound()
	{
		...
		lseg.insert(lseg.begin(),std::pair<unsigned,unsigned>(101,201));
		lseg.insert(lseg.begin(),std::pair<unsigned,unsigned>(150000,200000));
		lseg.insert(lseg.begin(),std::pair<unsigned,unsigned>(50000000,50220000));
		lseg.insert(lseg.begin(),std::pair<unsigned,unsigned>(21000,24000));
		...
		for(int i=0;i<100;i+=10)
		{
			...
			for(std::list<std::pair<unsigned,unsigned> >::iterator p=lseg.begin();p!=lseg.end();p++)
			{
				unsigned start=p->first-i;
				unsigned end=p->second+i;
				ve=blrEs.search(start,end);
				vb=blrBs.search(start,end);
...
CPPUNIT_TEST_SUITE_REGISTRATION(Test_BLRVecBinSearch);
#endif
