...
//for threads
std::vector<unsigned> threadedIdxSearch(BLRMemIdx* memIdx, unsigned seqnum, unsigned start, unsigned end, int verbose)
//void threadedIdxSearch(unsigned seqnum, unsigned start, unsigned end, int verbose)
{
	//std::cout<<"memIdx:"<<memIdx<<" seq:"<<seqnum<<" start:"<<start<<" end:"<<end<<" v:"<<v.size()<<std::endl;
	//std::mutex m;
	//std::lock_guard<std::mutex> lk(m);
	std::vector<unsigned> v;
    threadedSearch(memIdx->srange->vsrch[seqnum-1],start, end, v);
	//std::cout<<"found:"<<v.size()<<std::endl;
    return v;
};
