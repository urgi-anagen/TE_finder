...
void help ( char *programName )
{
  ...
  cout << "     -i: name of the input file (format='path')" << endl;
...
void pathnum2id( ifstream &inFile, ofstream &outFile, int verbose )
{
  ...
  while ( ! inFile.eof() )
	...
	break;
 	...
 	cout << line << endl;
      ...
      string qryName = tokens[1];
      string qryStart = tokens[2];
      string qryEnd = tokens[3];
      string sbjName = tokens[4];
      string sbjStart = tokens[5];
      string sbjEnd = tokens[6];
      string evalue = tokens[7];
      string score = tokens[8];
      string identity = tokens[9];
      string ID = tokens[0] + "-" + qryName + "-" + sbjName;
      if ( verbose > 1 )
	cout << ID << endl;
	...
	newPath = dID2count[ ID ];
      ...
      newPath_str << newPath;
      string outLine = newPath_str.str() + "\t" + qryName + "\t" + qryStart + "\t" + qryEnd + "\t" + sbjName + "\t" + sbjStart + "\t" + sbjEnd + "\t" + evalue + "\t" + score + "\t" + identity + "\n";
...
int main ( int argc, char **argv )
{
  ...
  pathnum2id( inFile, outFile, verbose );
