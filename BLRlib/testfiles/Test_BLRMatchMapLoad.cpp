...
void Test_BLRMatchMapLoad::test_readPath()
{
   ...
   matchMap.writeRpsList(sout_obs);
   matchMap.writeRpsListAttribute(sout_obs);
...
//  std::cout<<"match loaded: "<<std::endl;
//  Test_BLRMatchMapUtils::viewMapAlign(obsMapAlign);
  
  BLRMatchMap::MapAlign expMapAlign = Test_BLRMatchMapLoadUtils::createExpMapAlign_for_test_load();
...
void Test_BLRMatchMapLoad::test_loadPath(void)
{
	  ...
	  BLRMatcherParameter para = Test_BLRMatchMapUtils::createParameter();
	  BLRMatchMap matchMap1(&para),matchMap2(&para);
	  ...
	  bool cleanAfter = false; // if true path attributes make the test failed but this is expected!!
	  bool merged = false;
	  int verboseParameter = 0;
	  matchMap1.mapPath(joiningParameter, cleanBefore, cleanAfter, merged, verboseParameter);

	  //write/save expected
	  std::ostringstream sout_exp;
	  matchMap1.writeRpsList(sout_exp);
	  matchMap1.writeRpsListAttribute(sout_exp);

	  SDGString path_file = "match.align.join.path";
	  matchMap1.writePath(path_file);

	  //Test method
	  para.setPath_filename(path_file);
	  matchMap2.loadPath(path_file);

	  std::ostringstream sout_obs;
	  matchMap2.writeRpsList(sout_obs);
	  matchMap2.writeRpsListAttribute(sout_obs);

	  CPPUNIT_ASSERT_EQUAL(sout_exp.str(), sout_obs.str());
	  FileUtils::removeFile(path_file);
	  FileUtils::removeFile(path_file+".attr");
