...
void Test_F_grouper::test_3FullLengthFragments( void )
{
	SDGString alignFile = "dummyAlignFile";
	std::ofstream alignFileStream;
	FileUtils::openFile( alignFile, alignFileStream );
	RangePair rp1 = RangePair( "seq1\t1\t100\tseq2\t1\t100\t0.0\t100\t100" );
	rp1.writetxt( alignFileStream );
	RangePair rp2 = RangePair( "seq1\t1\t100\tseq3\t1\t100\t0.0\t100\t100" );
	rp2.writetxt( alignFileStream );
	RangePair rp3 = RangePair( "seq2\t1\t100\tseq3\t1\t100\t0.0\t100\t100" );
	rp3.writetxt( alignFileStream );
	alignFileStream.close();

	SDGString queryFile = "dummyQueryFile";
	SDGFastaOstream queryFileStream( queryFile );
//	SDGFastaBioSeq seq1("seq1","ATGCATACGACGATGCAGCGATGCA");
//	seq1.write( queryFileStream );
	queryFileStream.close();

	SDGString cmd = grouperFile + " -m " + alignFile;
	int returnStatus = std::system( cmd );

	bool exp = true;
	bool obs = ( returnStatus == EXIT_SUCCESS );
	CPPUNIT_ASSERT_EQUAL( exp, obs );

	FileUtils::removeFile( alignFile );
	FileUtils::removeFile( queryFile );
}

void Test_F_grouper::test_3FullLengthFragments_1Truncated( void )
{
	SDGString alignFile = "dummyAlignFile";
	std::ofstream alignFileStream;
	FileUtils::openFile( alignFile, alignFileStream );
	RangePair rp12 = RangePair( "seq1\t1\t100\tseq2\t1\t100\t0.0\t100\t100" );
	rp12.writetxt( alignFileStream );
	RangePair rp13 = RangePair( "seq1\t1\t100\tseq3\t1\t100\t0.0\t100\t100" );
	rp13.writetxt( alignFileStream );
	RangePair rp14a = RangePair( "seq1\t1\t60\tseq4\t1\t60\t0.0\t60\t100" );
	rp14a.writetxt( alignFileStream );
	RangePair rp14b = RangePair( "seq1\t81\t100\tseq4\t61\t80\t0.0\t20\t100" );
	rp14b.writetxt( alignFileStream );
	RangePair rp23 = RangePair( "seq2\t1\t100\tseq3\t1\t100\t0.0\t100\t100" );
	rp23.writetxt( alignFileStream );
	RangePair rp24a = RangePair( "seq2\t1\t60\tseq4\t1\t60\t0.0\t60\t100" );
	rp24a.writetxt( alignFileStream );
	RangePair rp24b = RangePair( "seq2\t81\t100\tseq4\t61\t80\t0.0\t20\t100" );
	rp24b.writetxt( alignFileStream );
	RangePair rp34a = RangePair( "seq3\t1\t60\tseq4\t1\t60\t0.0\t60\t100" );
	rp34a.writetxt( alignFileStream );
	RangePair rp34b = RangePair( "seq3\t81\t100\tseq4\t61\t80\t0.0\t20\t100" );
	rp34b.writetxt( alignFileStream );
	alignFileStream.close();
...
void Test_F_grouper::test_runAsScript_bigData( void ){
	...
	SDGString outFaFileNameWithTestName = "DmelChr4.fa-megablast-316416173_bigData.align.group.c0.95.fa";
        ...
        cmd= grouperFile + " -m "+ inputFileNameWithTestName + " -q " + genomeFileNameWithTestName + " -s " + genomeFileNameWithTestName +" -j -Z 3 -v 2";
	std::cout<<" "<<std::endl;
...
void Test_F_grouper::test_runAsScript_smallData_filter_length_40_46( void ){
	...
	SDGString outFaFileNameWithTestName = "DmelChr4.fa-megablast-316416173_smallData.align.group.c0.95.fa";
        ...
        cmd= grouperFile + " -m "+ inputFileNameWithTestName + " -q " + genomeFileNameWithTestName + " -s " + genomeFileNameWithTestName +" -j -Z 3";
	cmd = cmd + " -v 2";
	std::cout<<" "<<std::endl;
	std::cout<<cmd<<std::endl;
  	std::system(cmd);
...
void Test_F_grouper::test_runAsScript_loadPath_option_grouper_only( void ){
	...
	cmd = grouperFile + " -v 2 -d 10 -g 1 -c 2 -m "+ alignFileName + " -j -q " + genomeFileName;
	std::system(cmd);
	...
	cmd = grouperFile + " -d 10 -g 1 -p "+ gPathFileName + " -q " + genomeFileName;
	std::system(cmd);
...
void Test_F_grouper::test_runAsScript_loadPath_option( void ){
	std::cout<<"test_runAsScript_loadPath_option"<<std::endl;
	SDGString GROUPER_DATA_SUFFIX = "/TE_finder/grouper/";
	SDGString GROUPER_DATA = std::getenv("REPET_DATA") + GROUPER_DATA_SUFFIX;
	...
	cmd = "../grouper2.26 -g 1 -d 10 -j -p "+ pathFileName + " -q " + genomeFileName + " -G -1";
	std::system(cmd);
	...
	//run grouper with align file
	cmd = "../grouper2.26 -v 2 -g 1 -d 10 -c 2 -m "+ alignFileName + " -j -q " + genomeFileName + " -G -1";
	std::system(cmd);
	...
	SDGString chainsPathFileNameloadAlign = alignFileName+".chains.path";
	SDGString chainsPathFileNameloadPath = pathFileName + ".chains.path";
	...
	FileUtils::removeFile(faFileNameMatGrp);
	FileUtils::removeFile(txtFileNameMatGrp);
	FileUtils::removeFile(chainsPathFileNameloadPath);
	...
	FileUtils::removeFile(faFileNameGrp);
	FileUtils::removeFile(txtFileNameGrp);
	FileUtils::removeFile(chainsPathFileNameloadAlign);
